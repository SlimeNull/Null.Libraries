...
{
    public static class CalculatorPlusStatic
    {
        ...
        static char[] OperatorChars = "+-*/<>!=&|?:".ToArray();
        static char[] NumberChars = "0123456789.".ToArray();
        ...
        // 下面是分词核心
        static bool TryParseCalcTokens(ref char[] source, ref int offset, out List<CalcToken> result, bool inner = false, int level = 0)
        {
        ...
        static bool TryParseToken(ref char[] source, ref int offset, out CalcToken result)
        {
        ...
        static bool TryParseNumberToken(ref char[] source, ref int offset, out double result)
        {
            result = 0;
        ...
        static bool TryParseOperatorToken(ref char[] source, ref int offset, out CalcOperator result)
        {
            result = 0;
        ...
        static bool ITryGetOperator(string str, out CalcOperator result)
        {
            switch (str.ToString())
            {
                case "+":
        ...
        // 下面是运算核心
        static bool TryCalculateTokens(ref List<CalcToken> source, out double result, out List<string> errors)
        {
        ...
        static bool CheckOperator(CalcOperator opt, CalcOperator target)
        {
            return (opt & target) != 0;
        }
        static bool CheckType(ref List<CalcToken> source, CalcTokenType type, int index, int offset)
        {
        ...
        static bool TryGetTokenValue(ref List<string> errors, CalcToken token, out double result)
        {
        ...
        // 下面是封装好的, 供使用的方法
        public static bool TryParseCalcTokens(string expression, out List<CalcToken> result)
        {
            char[] srccs = expression.ToArray();
            int offset = 0;
            return TryParseCalcTokens(ref srccs, ref offset, out result);
        }
        public static bool TryCalculateTokens(List<CalcToken> tokens, out double result)
        {
            ...
            {
                CalculatorPlusStatic.errors.AddRange(errors);
                return false;
            }
        }
